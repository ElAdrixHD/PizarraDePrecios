...
import es.adrianmmudarra.pizarradeprecios.ui.producto.ProductoFargment;
import es.adrianmmudarra.pizarradeprecios.ui.subasta.SubastaFragment;

public class DashActivity extends BaseActivity implements BottomNavigationView.OnNavigationItemSelectedListener, CooperativaFragment.CooperativaInterface, ProductoFargment.ProductoInterface {

    FragmentManager fragmentManager;
    DashFragment dashFragment;
    CooperativaFragment cooperativaFragment;
    SubastaFragment subastaFragment;
    ProductoFargment productoFargment;
    ...
    private void setProductoFragment(){
        productoFargment = (ProductoFargment)fragmentManager.findFragmentByTag(ProductoFargment.FRAGMENT_ID);
        fragmentManager.popBackStack(null,FragmentManager.POP_BACK_STACK_INCLUSIVE);
        if (productoFargment == null){
            productoFargment = new ProductoFargment();
        }
        FragmentTransaction transaction = fragmentManager.beginTransaction();
        transaction.replace(R.id.container,productoFargment,ProductoFargment.FRAGMENT_ID);
        transaction.addToBackStack(null);
        transaction.commit();
    }

    @Override
    public void setSubastaProductoFragment(int id_prod) {
